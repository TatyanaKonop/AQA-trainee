...
import pytest
from API.random_sentence import generate_random_string


@pytest.mark.parametrize('execution_number', range(3))  # Execute test 3 times
def test_input_random_text(browser, execution_number, send_link,
                           send_number_of_length):  # Test checking whether input sentence is equal output one
    ...
    value_input = generate_random_string(send_number_of_length)
    values_button = instance_text_input_page.input_random_text(
        value_input)  # Execute method page, receive a changed name of button
